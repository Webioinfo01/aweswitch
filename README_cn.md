...
- **zcode** — provider 条目（base URL、环境变量 key 引用、显示名）及其**完整模型列表**按 upsert 写入 `~/.zcode/v2/config.json`。zcode 的一个 provider 只支持一种 API 格式，因此 profile 只能声明 `ZCODE_CHAT_MODEL`（chat completions，provider `kind: openai-compatible`）**或** `ZCODE_RESPONSES_MODEL`（Responses API，provider `kind: openai`），二者互斥——需要两种格式就拆成两个 profile。模型字段的 key 顺序即选择器顺序，每次 apply 都会重写。每个受管的 chat 模型（`ZCODE_CHAT_MODEL`）还会补写一个 fill-only 的默认 reasoning 块（`low` / `medium` / `high` / `xhigh` / `max` 五档，默认 `max`），让思考强度选择器显示出来——zcode 对没有 reasoning 块的 chat 模型会隐藏该选择器；Responses API（`ZCODE_RESPONSES_MODEL`）的模型走 zcode 原生档位选择器，不做改写。手写的 reasoning 块整体保留（已有的 variants 列表不会被增删或重排），`"reasoning": false` 表示显式关闭。zcode 是桌面 GUI 应用，因此 zcode profile 只支持 apply，不支持启动。`--zcode` 会同步全部 zcode profile；aweswitch 用 `.aweswitch-managed-providers.json` 登记自己管理的 provider，默认只警告孤儿条目，显式加 `--prune`（`orphans` / `all` / 点名）才清理；`--dry-run` 没有 zcode 侧预览。
