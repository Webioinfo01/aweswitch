...
- `ZCODE_SUBAGENT_MODEL` — a single model ID written into zcode's built-in agent overrides (`general-purpose` + `Explore` in `~/.zcode/v2/agents-state.json`). The override is released only when no zcode profile declares the field anymore, falling back to inheriting the session model.
...
- **Model order is the picker order.** The key order of `OPENCODE_MODEL` (and of the model fields in `env`) is what OpenCode's model picker shows. `aweswitch apply` rewrites the provider's model keys to the configured order, so to reshuffle the picker, reorder the keys in the aweswitch config and run `aweswitch apply oc-aweshare` — no hand-editing of `opencode.json`. Launch stays additive and never reorders.
