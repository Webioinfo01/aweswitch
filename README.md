...
- **zcode** — the provider entry (base URL, env key reference, display name) and its **full model list** are upserted into `~/.zcode/v2/config.json`. zcode supports exactly one API format per provider, so a profile takes `ZCODE_CHAT_MODEL` (chat completions, provider `kind: openai-compatible`) **or** `ZCODE_RESPONSES_MODEL` (Responses API, provider `kind: openai`) — never both; split them into two profiles instead. The key order of the model field is the picker order and is rewritten on every apply. Every managed chat model (`ZCODE_CHAT_MODEL`) also gets a fill-only default reasoning block (`low` / `medium` / `high` / `xhigh` / `max`, `max` selected) so the thought-level picker shows up — zcode hides it for a chat model with no reasoning block, while Responses-API profiles are left to zcode's own native effort picker. Hand-written reasoning blocks win wholesale; `"reasoning": false` opts out. zcode is a desktop GUI app, so zcode profiles are apply-only and do not support launch mode. `--zcode` syncs every zcode profile; managed providers are tracked in `.aweswitch-managed-providers.json`, with orphan warnings by default and opt-in cleanup via `--prune` (`orphans`, `all`, or names; `--dry-run` has no zcode preview).
